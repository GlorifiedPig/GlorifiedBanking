...
surface.CreateFont("EclipsePrinterPlyName", {
    font = "Arial",
    size = 30,
    weight = 700,
    antialias = true
    ...
    if imgui.Entity3D2D(self, padpos, padang, 0.03, 150, 120) then
        for i=1,3 do
            for j=1,4 do
                ...
                if imgui.IsHovering(keyx, keyy, keyw, keyh) then
                    local col = keyhovercol
...
function ENT:PlayGBAnim(type)
    ...
    if type == GB_ANIM_CARD_IN then self.CardPos = 60 end
    if type == GB_ANIM_CARD_OUT then self.CardPos = 0 end

    if type == GB_ANIM_MONEY_IN or type == GB_ANIM_MONEY_OUT then
        self.MoneyPos = Vector()
        if type == GB_ANIM_MONEY_IN then self.MoneyPos:Set(moneyoutpos) else self.MoneyPos:Set(moneyinpos) end
        ...
        if IsValid(self.MoneyModel) then
            timer.Simple(0, function() self.MoneyModel:Remove() end)            
        end
    end
end
