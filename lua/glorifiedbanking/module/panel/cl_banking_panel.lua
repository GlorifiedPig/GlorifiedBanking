        ...
        if isnumber( withdrawAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableWithdraw and withdrawAmount <= glorifiedbanking.config.MAX_WITHDRAWAL then
                    ...
                    WithdrawFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully withdrawn " .. DarkRP.formatMoney( withdrawAmount ), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
        ...
        if isnumber( depositAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableDeposit and depositAmount <= glorifiedbanking.config.MAX_DEPOSIT then
                    ...
                    DepositFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully deposited " .. DarkRP.formatMoney( depositAmount ), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
        ...
        if isnumber( transferAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableTransfer and transferAmount <= glorifiedbanking.config.MAX_TRANSFER then
                    ...
                    TransferFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully transferred " .. DarkRP.formatMoney( transferAmount ) .. " to " .. transferringPlayer:Nick(), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
