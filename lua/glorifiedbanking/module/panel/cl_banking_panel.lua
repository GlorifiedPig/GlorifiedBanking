        ...
        if isnumber( withdrawAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableWithdraw and withdrawAmount <= glorifiedbanking.config.MAX_WITHDRAWAL then
                    ...
                    WithdrawFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully withdrawn $" .. DarkRP.formatMoney( withdrawText:GetText() ), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
                elseif affordableWithdraw and withdrawAmount > glorifiedbanking.config.MAX_WITHDRAWAL then
                    WithdrawFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You cannot withdraw more than $" .. DarkRP.formatMoney( glorifiedbanking.config.MAX_WITHDRAWAL ) .. " at a time.", NOTIFY_ERROR, 5)
                    surface.PlaySound("buttons/button2.wav")
        ...
        if isnumber( depositAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableDeposit and depositAmount <= glorifiedbanking.config.MAX_DEPOSIT then
                    ...
                    DepositFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully deposited $" .. DarkRP.formatMoney( depositText:GetText() ), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
                elseif affordableDeposit and depositAmount > glorifiedbanking.config.MAX_DEPOSIT then
                    DepositFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You cannot deposit more than $" .. DarkRP.formatMoney( glorifiedbanking.config.MAX_DEPOSIT ) .. " at a time.", NOTIFY_ERROR, 5)
                    surface.PlaySound("buttons/button2.wav")
        ...
        if isnumber( transferAmount ) then
            ...
            timer.Simple( ply:Ping() / 1000 + 0.1, function()
                if affordableTransfer and transferAmount <= glorifiedbanking.config.MAX_TRANSFER then
                    ...
                    TransferFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You have successfully transferred $" .. DarkRP.formatMoney( tostring( transferAmount ) ) .. " to " .. transferringPlayer:Nick(), NOTIFY_GENERIC, 5)
                    surface.PlaySound("buttons/button14.wav")
                elseif affordableTransfer and transferAmount > glorifiedbanking.config.MAX_TRANSFER then
                    TransferFrame:Close()
                    Frame:Close()
                    notification.AddLegacy("You cannot transfer more than $" .. DarkRP.formatMoney( glorifiedbanking.config.MAX_TRANSFER ) .. " at a time.", NOTIFY_ERROR, 5)
                    surface.PlaySound("buttons/button2.wav")
...
local function OpenBankingPanel()
    ...
    timer.Simple( ply:Ping() / 1000 + 0.1, function()
        ...
        local balW, balH = surface.GetTextSize( "Current Balance: $" .. DarkRP.formatMoney( bankBalance ) )
		...
		BalanceLabel:SetText( "Current Balance: $" .. DarkRP.formatMoney( bankBalance ) )
